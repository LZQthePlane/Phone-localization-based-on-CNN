...
class Cnn(object):
    ...
    def build_net(self, keep_prob, with_bn, x):
        input = tf.constant(x, name='inputs')
        relu = tf.nn.relu
        ...
        layer1 = self.conv_pool_layer(pre_layer=input, filters=8, conv_ksize=3, conv_strides=(2, 2), po_ksize=4,
    ...
    def loss(self, step, x, y, keep_prob, with_bn):
        pred = self.build_net(keep_prob=keep_prob, with_bn=with_bn, x=x)
        loss = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(pred - y), axis=1)), axis=0)
        return loss


    def train_test(self, keep_prob, with_bn, splited_data, save_path=None):
        x_train, x_test = splited_data[0], splited_data[1]
        y_train, y_test = splited_data[2], splited_data[3]
        # sess = start_sess()
        # pred = self.build_net(keep_prob=keep_prob, with_bn=with_bn, x=x_train)
        # loss = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(pred - y_train), axis=1)), axis=0)
        ...
        # correct_pre = tf.divide(tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(pred, y)), axis=1)), 255) < 0.05
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            for i in range(self.epochs):
                for x_batch, y_batch in batches:
                    optimizer.minimize(self.loss(i, x=x_batch, y=y_batch, keep_prob=keep_prob, with_bn=with_bn))
                train_correct_pre = tf.divide(tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(pred, y_train)), axis=1)), 255) < 0.05
                accuracy = tf.reduce_mean(tf.cast(train_correct_pre, tf.float32))
                # print('epoch {0}'.format(step + 1) + ':\t train acc:' + str(round(train_correct_pre, 4)) + ';\t' + 'test acc:' + str(round(test_acc, 4)))
                print('epoch {0}'.format(i + 1) + ':\t train acc:' + str(round(accuracy, 4)))
